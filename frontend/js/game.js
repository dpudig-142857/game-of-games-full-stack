...
function timeDisplay() {
    ...
    if (startTime) {
        ...
        if (mins < 60) {
            ...
            elapsedTime = `⏱️ ${days}d ${hours % 24}h ${pad(mins % 60)}m`;
        }
...
async function createTournament(curr) {
    ...
    if (res.ok) {
        ...
        right.className = 'box';
        right.style.backgroundColor = curr_colour.rgba;
        right.style.borderColor = colour.rgba;
        right.style.color = colour.text;
        ...
        bracketDiv.style.borderColor = colour.rgba;
        bracketDiv.style.color = colour.text;
...
async function initialise(sessionId) {
    try {
        ...
        console.log('refreshed = ', refreshedGames);
